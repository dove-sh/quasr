...
export default async function({cli}:CliFunctionContext){
    cli('restart', async (args)=>{
        let loader = 
        loading({text: "restarting application...", interval: cli_spinners.arc.interval, stream: process.stdout, frames: cli_spinners.arc.frames});
        loader.start();
        try{
            await (args.ctx.current_app as StartableApplication).restart()
        }
        catch(e){
            loader.stop();
        ...
        loader.stop();
        print(`${cli_colors.bgGreen}${cli_colors.black} success ${cli_colors.reset} restarted application`);
    });
